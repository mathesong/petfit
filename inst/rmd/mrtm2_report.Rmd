...
# Dynamically select available parameters. k2prime is not fitted in MRTM2, it
# is a prior value, and k2 is R1 times that prior, so neither adds anything to
# R1. k2a does: it carries the binding as well as the flow. R1, k2 and k2a are
# only returned when no t* was applied, so intersect() drops them when absent.
available_params <- intersect(colnames(par_table), c("R1", "k2a", "BPnd"))
