...
if (nrow(raw_data) == 0) {
  stop("No PET measurements found in bids_dir. ",
       "Ensure _pet.json sidecar files are present alongside _blood.tsv files.")
}
...
if (nrow(save_data) == 0) {
  stop("No blood data matched the TAC files. ",
       "Check that blood file BIDS entities (sub, ses, trc) match the TAC files.")
}

walk2(save_data$blooddata, save_data$filename, blooddata2inputfunction_tsv)

# Verify inputfunction files were created
inputfunction_check <- list.files(analysis_folder, pattern = "_inputfunction\\.tsv$", recursive = TRUE)
if (length(inputfunction_check) == 0) {
  stop("No inputfunction files were created from the blood data. ",
       "Check that blood data in bids_dir is valid.")
}
