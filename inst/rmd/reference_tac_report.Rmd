---
title: "Reference TAC Report"
output:
  html_document:
    toc: true
    toc_float: true
    toc_depth: 2
    code_folding: hide
params:
  cores: 1
  analysis_folder: NULL
  bids_dir: NULL
  blood_dir: NULL
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = TRUE, message = FALSE)
# Warnings go to the console/step log, not into this report: dplyr's
# many-to-many join warning is the signal that a duplicated record has
# multiplied TAC rows, and it used to be discarded outright.
petfit::divert_report_warnings()

library(ggplot2)
library(dplyr)
library(tidyr)
library(readr)
library(purrr)
library(tibble)
library(stringr)
library(kinfitr)
library(knitr)
library(jsonlite)
library(plotly)
library(crosstalk)
library(htmltools)
library(scales)
library(furrr)

# Fixed seed. The model fits use multstart, which draws its starting parameter
# sets at random; furrr_options(seed = TRUE) makes those draws parallel-safe but
# not reproducible, because it derives its streams from whatever state the
# session happens to be in. Seeding here makes a re-run of the same analysis
# give the same numbers, whatever `cores` is set to. Vary it deliberately if you
# want to know how seed-dependent a fit is.
set.seed(123)

cores <- params$cores %||% 1
if (cores > 1) {
  if (future::supportsMulticore()) {
    future::plan(future::multicore, workers = cores)
  } else {
    future::plan(future::multisession, workers = cores)
  }
} else {
  future::plan(future::sequential)
}

theme_set(theme_light())

# ---------------------------------------------------------------------------
# Reference TAC spline fitting helpers
#
# kinfitr::spline_tac() takes the basis dimension as `k`, following mgcv, where
# k = -1 means "choose the dimension automatically". The configured
# ReferenceTAC$spline_df is that k. Anything negative or missing is therefore a
# request for the automatic choice, not a degenerate basis.
# ---------------------------------------------------------------------------

# A basis of 0 or 1 dimensions cannot describe anything but a level, and a
# steady-state reference TAC can fail spline basis construction outright. In
# both cases fall back to the measured reference TAC itself, unsmoothed: this is
# the "raw" fitting method the app already offers, so the measurement is still
# modelled with a real reference input, only without the smoothing. 
raw_reference_tac_fit <- function(t_tac, tac, reason = "") {
  structure(
    list(
      tacs = tibble(
        Time = t_tac,
        TAC = tac,
        TAC_fitted = tac
      ),
      # No curve was fitted, so there is no fitted time at which it starts to
      # rise. NA says that; a number here would be invented.
      par = tibble(t0 = NA_real_),
      fit_message = reason,
      petfit_raw_fallback = TRUE
    ),
    class = c("petfit_raw_reference_tac_fit", "list")
  )
}

fit_reference_spline_tac <- function(t_tac, tac, weights = NULL, spline_df = NULL) {
  spline_df <- suppressWarnings(as.numeric(spline_df %||% NA_real_))

  # Negative or missing: let kinfitr pick the basis dimension itself.
  auto_df <- is.na(spline_df) || spline_df < 0

  if (!auto_df && spline_df < 2) {
    return(raw_reference_tac_fit(
      t_tac = t_tac,
      tac = tac,
      reason = str_glue("Spline degrees of freedom ({spline_df}) is below 2, ",
                        "which cannot form a spline basis; using the raw ",
                        "(unsmoothed) reference TAC.")
    ))
  }

  spline_args <- list(
    t_tac = t_tac,
    tac = tac,
    weights = weights
  )

  if (!auto_df) {
    spline_args$k <- spline_df
  }

  tryCatch(
    do.call(kinfitr::spline_tac, spline_args),
    error = function(e) {
      raw_reference_tac_fit(
        t_tac = t_tac,
        tac = tac,
        reason = str_glue("Spline fit failed ({conditionMessage(e)}); using the ",
                          "raw (unsmoothed) reference TAC.")
      )
    }
  )
}

# spline_tac() returns its fitted values on its own internal time grid, which
# tidyinput_ref() may have extended with a frame at time zero. Joining that grid
# back onto the PET frames by equality silently drops rows, and plotting it
# directly draws a curve starting at zero for a delayed-start acquisition.
# Interpolate back onto the original frame midpoints instead.
reference_spline_predictions <- function(ref_fit, ref_tacs) {
  if (!all(c("Time", "TAC_fitted") %in% colnames(ref_fit$tacs))) {
    stop("Spline fit output does not contain Time and TAC_fitted columns.")
  }

  original_time <- ref_tacs$frame_mid / 60
  fit_time <- ref_fit$tacs$Time

  if (!(min(fit_time, na.rm = TRUE) <= min(original_time, na.rm = TRUE) &&
        max(fit_time, na.rm = TRUE) >= max(original_time, na.rm = TRUE))) {
    shifted_fit_time <- fit_time + min(original_time, na.rm = TRUE) - min(fit_time, na.rm = TRUE)

    if (min(shifted_fit_time, na.rm = TRUE) <= min(original_time, na.rm = TRUE) &&
        max(shifted_fit_time, na.rm = TRUE) >= max(original_time, na.rm = TRUE)) {
      fit_time <- shifted_fit_time
    }
  }

  fitted_tac <- approx(
    x = fit_time,
    y = ref_fit$tacs$TAC_fitted,
    xout = original_time,
    rule = 2,
    ties = "ordered"
  )$y

  ref_tacs %>%
    transmute(
      frame_start,
      frame_mid,
      RefTAC = RefTAC,
      RefTAC_fitted = fitted_tac
    )
}

plot_reference_spline_fit <- function(ref_fit, ref_tacs,
                                      units = tac_units$radioactivity) {
  plot_data <- reference_spline_predictions(ref_fit, ref_tacs) %>%
    mutate(
      plot_time = frame_mid / 60,
      plot_start = frame_start / 60
    )

  ggplot(plot_data, aes(x = plot_time, y = RefTAC)) +
    geom_point(colour = "black") +
    geom_line(aes(y = RefTAC_fitted), colour = "black") +
    coord_cartesian(xlim = range(plot_data$plot_start, plot_data$plot_time, na.rm = TRUE)) +
    # A TAC is a concentration, and the units are the analysis's own rather than
    # an assumed kBq.
    labs(x = "Time (min)", y = paste0("Radioactivity (", units, ")"))
}


# Use parameters (from params or debugging section above)
analysis_folder <- params$analysis_folder
# The units of the TACs everything downstream is derived from, recorded by
# the region definition step. Read rather than assumed, so that the sidecars
# written below cannot claim units the data does not have.
tac_units <- petfit_tac_units(analysis_folder)
bids_dir <- params$bids_dir

# Load configuration from standard location
config_path <- file.path(analysis_folder, "desc-petfitoptions_config.json")
config <- NULL
if (file.exists(config_path)) {
  tryCatch({
    config <- jsonlite::fromJSON(config_path)
    config <- coerce_bounds_numeric(config)
    # cat("Loaded config from:", config_path, "\n")
  }, error = function(e) {
    cat("Warning: Could not load config file:", e$message, "\n")
  })
} else {
  cat("Config file not found at:", config_path, "\n")
}
```

This report summarises the reference TAC preparation step.

# Analysis Configuration

**Analysis folder:** `r params$analysis_folder`

**Generated on:** `r format(Sys.time(), "%Y-%m-%d %H:%M:%S")`

# Reference TAC Configuration

```{r reference-config}
if (!is.null(config$ReferenceTAC)) {
  ref_tac <- config$ReferenceTAC

  # Method mapping for display
  fitting_method_display <- switch(ref_tac$fitting_method %||% "raw",
    "raw" = "Raw Reference TAC (no fitting)",
    "feng1tc" = "Feng+1TC Reference Model",
    "spline" = "Spline Model",
    "Unknown method"
  )

  # Weighting method display
  weights_method_display <- switch(ref_tac$weights_method %||% "same_as_target",
    "same_as_target" = "Same weights as target TACs",
    "0" = "0. Uniform (all weights = 1)",
    "1" = "1. frame_dur / tac_uncor",
    "2" = "2. sqrt(frame_dur * tac_uncor)",
    "3" = "3. sqrt(frame_dur) / tac",
    "4" = "4. sqrt(frame_dur)",
    "5" = "5. frame_dur * exp(-ln(2)/halflife)",
    "6" = "6. frame_dur / tac",
    "7" = "7. frame_dur",
    "8" = "8. frame_dur^2 / tac_uncor",
    "9" = "9. (frame_dur^2 / (frame_dur * tac)) * corrections^2",
    "custom" = "Custom formula",
    "Unknown method"
  )

  # Build configuration items list
  config_items <- list(
    "Reference Region" = ref_tac$region %||% "Not specified",
    "Fitting Method" = fitting_method_display
  )

  # Add noise approximation for raw method
  if ((ref_tac$fitting_method %||% "raw") == "raw") {
    noise_approx <- ref_tac$noise_approximation %||% FALSE
    if (!is.logical(noise_approx) || noise_approx == "") {
      noise_approx <- FALSE
    }
    config_items[["Noise Approximation"]] <- ifelse(noise_approx, "Yes (spline-based)", "No")
  }

  # Add spline df for spline method
  if ((ref_tac$fitting_method %||% "raw") == "spline") {
    config_items[["Spline Degrees of Freedom"]] <- ref_tac$spline_df %||% "Not specified"
  }

  # Add weighting info for fitted methods
  if ((ref_tac$fitting_method %||% "raw") != "raw") {
    config_items[["Reference Weighting Method"]] <- weights_method_display

    # Only show custom formula if user selected custom weights
    if ((ref_tac$weights_method %||% "same_as_target") == "custom") {
      config_items[["Custom Formula"]] <- ref_tac$weights_custom_formula %||% "Not specified"
    }

    if ((ref_tac$weights_method %||% "same_as_target") != "same_as_target" &&
        !is.null(ref_tac$weights_minweight) && ref_tac$weights_minweight != "") {
      config_items[["Minimum Weight"]] <- ref_tac$weights_minweight
    }
  }

  config_df <- tibble(
    Parameter = names(config_items),
    Value = map_chr(config_items, as.character)
  )

  kable(config_df, caption = "Reference TAC Configuration")

} else {
  cat("No reference TAC configuration found in the analysis file.")
}
```

```{r setup-conditions, echo=FALSE}
# Set up conditional execution based on reference TAC method
fitting_method <- config$ReferenceTAC$fitting_method %||% "raw"
noise_approximation <- config$ReferenceTAC$noise_approximation %||% FALSE

# Convert empty string to FALSE for backward compatibility
if (!is.logical(noise_approximation) || noise_approximation == "") {
  noise_approximation <- FALSE
}

# Define execution conditions
use_raw_refs <- fitting_method == "raw"
use_feng1tc <- fitting_method == "feng1tc"
use_spline <- fitting_method == "spline"
do_noise_approx <- use_raw_refs && noise_approximation
```


# Loading Data

## Loading TAC Data

```{r load-tacs}
tacs_files <- list.files(analysis_folder,
                        pattern = "*_desc-combinedregions_tacs.tsv",
                        recursive = TRUE)

if (length(tacs_files) == 0) {
  stop("No TAC files found in analysis folder: ", analysis_folder)
}

# Define column types to ensure BIDS identifiers are always character
bids_col_types <- cols(
  sub = col_character(),
  ses = col_character(),
  trc = col_character(),
  rec = col_character(),
  task = col_character(),
  run = col_character(),
  pet = col_character(),
  segmentation = col_character(),
  region = col_character(),
  .default = col_guess()
)

# Read TAC files - BIDS identifiers are already in the TSV files,
# so we only need the filename for tracking
tac_data <- tibble(filename = tacs_files) %>%
  mutate(tacs = map(filename, ~read_tsv(file.path(analysis_folder, .x),
                                        col_types = bids_col_types))) %>%
  unnest(tacs)
```


## Extract Reference Region

```{r extract-reference, results='asis'}
ref_region_name <- config$ReferenceTAC$region %||% ""

if (ref_region_name == "") {
  stop("No reference region specified in configuration")
}

# Check if reference region exists in data
available_regions <- unique(tac_data$region)

if (!ref_region_name %in% available_regions) {
  stop("Reference region '", ref_region_name, "' not found in data. Available regions: ",
       paste(available_regions, collapse = ", "))
}

unique_pets <- length(unique(tac_data$pet))

pet_with_refs <- tac_data %>% 
  filter(region == ref_region_name) %>% 
  filter(!duplicated(pet)) %>% 
  pull(pet) %>% 
  length()

str_glue("Reference region **{ref_region_name}** found for {pet_with_refs} of {unique_pets} PET measurements.")
```


```{r}
ref_tac_data <- tac_data %>%
  filter(region == ref_region_name) %>%
  rename(tac_filename = filename) %>%
  select(-region) %>%
  rename(RefTAC = TAC)

target_tac_data <- tac_data %>%
  filter(region != ref_region_name) %>%
  rename(tac_filename = filename)
```


# Reference Weights Calculation

```{r setup-ref-weights-conditions, echo=FALSE}
# Determine if we need to calculate reference weights
# Only needed for fitted methods (feng1tc, spline) and when not using same weights as target
calculate_ref_weights <- (fitting_method %in% c("feng1tc", "spline")) &&
                         ((config$ReferenceTAC$weights_method %||% "same_as_target") != "same_as_target")

use_custom_ref_weights <- calculate_ref_weights &&
                          ((config$ReferenceTAC$weights_method %||% "same_as_target") == "custom")
use_defined_ref_weights <- calculate_ref_weights &&
                           ((config$ReferenceTAC$weights_method %||% "same_as_target") != "custom")
```

```{r, eval=calculate_ref_weights, echo=calculate_ref_weights, results='asis'}
str_glue("## Reference Region Weighting

Calculating weights for the reference region fitting using method: **{config$ReferenceTAC$weights_formula %||% 'Not specified'}**")
```

```{r load-weight-files, eval=calculate_ref_weights, echo=calculate_ref_weights}
# Load existing weight files created by weights_report.Rmd
weight_files <- list.files(analysis_folder,
                          pattern = "*_desc-weights_weights.tsv",
                          recursive = TRUE,
                          full.names = TRUE)

if (length(weight_files) == 0) {
  stop("No weight files found. Please run the weights calculation step first.")
}

# Read weight files and extract TAC data
ref_weights_tacdata <- tibble(
  filename = weight_files) %>%
  mutate(basename = basename(filename)) %>%
  mutate(weight_data = map(filename, ~read_tsv(.x, show_col_types = FALSE))) %>%
  unnest(weight_data) %>%
  select(pet, frame_start, frame_end, frame_dur, frame_mid, TAC)
```

```{r ref-weights-radioisotope, eval=calculate_ref_weights, echo=calculate_ref_weights}
# Extract radioisotope and halflife from Weights config (inherited from target weights)
radioisotope <- config$Weights$radioisotope %||% "C11"
halflife_raw <- config$Weights$halflife %||% ""

# Convert empty strings back to NULL for function calls
halflife <- if(halflife_raw == "" || is.null(halflife_raw)) NULL else as.numeric(halflife_raw)

# Prepare radioisotope and half-life
if (radioisotope == "Other" && !is.null(halflife)) {
  radionuclide_halflife <- halflife
  radioisotope_for_uncorrection <- "C11"  # Use as placeholder, actual halflife will override
} else {
  radionuclide_halflife <- NULL
  radioisotope_for_uncorrection <- radioisotope
}
```

```{r ref-weights-custom-prep, eval=use_custom_ref_weights, echo=use_custom_ref_weights}
# Prepare data for custom weights calculation
ref_weights_data <- ref_weights_tacdata %>%
  rename(tac = TAC) %>%
  mutate(tac_uncor = decay_uncorrect(frame_start, frame_end, tac,
                                     radioisotope = radioisotope_for_uncorrection,
                                     radionuclide_halflife = radionuclide_halflife)) %>%
  mutate(corrections = tac_uncor/tac)
```

```{r gen-ref-weights-custom, eval=use_custom_ref_weights, echo=use_custom_ref_weights}
# Apply custom weight formula
apply_weight_formula <- function(data, formula_string) {
  data %>%
    mutate(calcweights = eval(parse(text = formula_string)))
}

ref_weights_output <- apply_weight_formula(ref_weights_data, config$ReferenceTAC$weights_formula) %>%
  select(-tac_uncor, -corrections) %>%
  rename(TAC = tac)
```

```{r minweight-ref-weights-custom, eval=use_custom_ref_weights, echo=use_custom_ref_weights}
# Apply post-processing for custom weights
minweight <- as.numeric(config$ReferenceTAC$weights_minweight %||% 0.25)

ref_weights_final <- ref_weights_output %>%
  mutate(
    ref_weights = {

      # Outlier protection for max weight
      maxweight <- max(calcweights, na.rm = TRUE)
      maxweights <- tail(calcweights[order(calcweights)], 5)
      if (min(maxweights, na.rm = TRUE) < 0.5 * maxweight) {
        maxweight <- median(maxweights)
      }

      # Normalize weights
      calcweights[calcweights > maxweight] <- maxweight
      calcweights <- calcweights / maxweight

      # Apply minweight correction
      minweights <- rep(minweight, length(calcweights))
      if (any(calcweights < minweights)) {
        min_calcweight <- min(calcweights[frame_dur != 0], na.rm = TRUE)
        calcweights <- calcweights - min_calcweight / (1 - min_calcweight)
        calcweights <- minweights + calcweights * (1 - minweights)
      }

      calcweights[frame_dur == 0] <- 0
      calcweights
    }
  ) %>%
  select(-calcweights)
```

```{r gen-ref-weights-defined, eval=use_defined_ref_weights, echo=use_defined_ref_weights}
# Use predefined weighting methods
if (as.numeric(config$ReferenceTAC$weights_method) == 0) {

  # Method 0 (uniform weights)
  ref_weights_output <- ref_weights_tacdata %>%
    mutate(ref_weights = 1)

} else {

  # Using pre-defined weighting methods
  ref_weights_output <- ref_weights_tacdata %>%
    mutate(ref_weights = weights_create(t_start = frame_start/60,
                                    t_end = frame_end/60,
                                    TAC,
                                    radioisotope = radioisotope_for_uncorrection,
                                    method = as.numeric(config$ReferenceTAC$weights_method),
                                    minweight = as.numeric(config$ReferenceTAC$weights_minweight %||% 0.25),
                                    radionuclide_halflife = radionuclide_halflife))

}

ref_weights_final <- ref_weights_output
```

```{r normalise-ref-weights, eval=calculate_ref_weights, echo=calculate_ref_weights}
# Normalise reference weights to mean 1
ref_weights_final <- ref_weights_final %>%
  group_by(pet) %>%
  mutate(ref_weights = ref_weights/mean(ref_weights)) %>%
  ungroup()
```


```{r, eval=!calculate_ref_weights && fitting_method != "raw", echo=FALSE, results='asis'}
str_glue("## Reference Region Weighting

Using the same weights as the target TACs for reference region fitting.")
```

```{r, eval=!calculate_ref_weights && fitting_method != "raw", echo=!calculate_ref_weights && fitting_method != "raw"}
weights_files <- list.files(analysis_folder,
                        pattern = "*_weights.tsv",
                        recursive = TRUE)

ref_weights_final <- tibble(
  filename = weights_files) %>%
  mutate(basename = basename(filename)) %>%
  mutate(attributes = map(filename, kinfitr::bids_filename_attributes)) %>%
  unnest(attributes) %>%
  group_by(filename) %>%
  mutate(tacs = map(filename, ~read_tsv(file.path(analysis_folder, .x), show_col_types = FALSE))) %>%
  ungroup() %>%
  unnest(tacs) %>%
  select(-basename, -measurement, -desc, -TAC, -filename) %>% 
  rename(ref_weights = weights)


```


# Reference TAC Processing

```{r, eval=use_raw_refs, echo=use_raw_refs, results='asis'}
str_glue("## Raw Reference TAC

Using raw (unfitted) reference TAC for modelling.")
```

```{r raw-reference-processing, eval=use_raw_refs, echo=use_raw_refs}
# For raw reference TAC, we simply use the data as-is
# No fitting is performed
ref_tac_preddata <- ref_tac_data %>%
  group_by(across(intersect(colnames(.), c("sub", "ses", "trc", "rec", "task",
                                           "run", "pet", "tac_filename")))) %>%
  nest(.key = "ref_tacs") %>%
  ungroup() %>%
  mutate(ref_tacs_json = map(1:n(), ~c(list(
    Description = "Raw Reference TAC (no fitting applied).",
    ModelName = "Raw",
    FittingMethod = "raw",
    AdditionalModelDetails = str_glue("Reference region: {ref_region_name}. No fitting performed.")),
    bids_column_units(list(
      frame_start = "Frame start time",
      frame_end = "Frame end time",
      frame_dur = "Frame duration",
      frame_mid = "Frame midpoint time",
      RefTAC = "Original reference TAC (unmodified)"),
      radioactivity_units = tac_units$radioactivity,
      time_units = tac_units$time))))
```


```{r, eval=do_noise_approx, echo=do_noise_approx, results='asis'}
str_glue("### Noise Approximation Analysis

Comparing noise levels in reference region to target regions using spline-based estimation. ",
"Here we can compare whether the reference region has much greater approximated noise compared ",
"to the target regions. We should be concerned with our reference region if the ",
"RSS Ratio consistently << 1, implying much more noise in the reference region.")
```


```{r noise-approximation, eval=do_noise_approx, echo=do_noise_approx, fig.height=6, fig.width=9}
weights_files <- list.files(analysis_folder,
                        pattern = "*_weights.tsv",
                        recursive = TRUE)

weights_data <- tibble(
  filename = weights_files) %>%
  mutate(basename = basename(filename)) %>%
  mutate(attributes = map(filename, kinfitr::bids_filename_attributes)) %>%
  unnest(attributes) %>%
  group_by(filename) %>%
  mutate(tacs = map(filename, ~read_tsv(file.path(analysis_folder, .x), show_col_types = FALSE))) %>%
  ungroup() %>%
  unnest(tacs) %>%
  select(-basename, -measurement, -desc, -TAC, -filename)

ref_tac_data_noisecalc <- ref_tac_data %>%
  inner_join(weights_data) %>%
  group_by(across(intersect(colnames(.), c("sub", "ses", "trc", "rec", "task",
                                           "run", "pet", "filename")))) %>%
  nest(.key = "ref_tacs") %>%
  ungroup() %>%
  mutate(splinefit = future_map(ref_tacs, ~spline_tac(t_tac   = .x$frame_mid/60,
                                               tac     = .x$RefTAC,
                                               weights = .x$weights),
                                .options = furrr_options(seed = TRUE))) %>%
  mutate(RSS = map_dbl(splinefit, ~sum((weights(.x)/mean(weights(.x))) *
                                         (.x$tacs$TAC_fitted - .x$tacs$TAC)^2)))

target_tac_data_noisecalc <- target_tac_data %>%
  inner_join(weights_data) %>%
  group_by(across(intersect(colnames(.), c("sub", "ses", "trc", "rec", "task",
                                           "run", "pet", "filename", "region")))) %>%
  nest(.key = "target_tacs") %>%
  ungroup() %>%
  mutate(splinefit = future_map(target_tacs, ~spline_tac(t_tac   = .x$frame_mid/60,
                                                  tac     = .x$TAC,
                                                  weights = .x$weights),
                                .options = furrr_options(seed = TRUE))) %>%
  mutate(RSS = map_dbl(splinefit, ~sum((weights(.x)/mean(weights(.x))) *
                                         (.x$tacs$TAC_fitted - .x$tacs$TAC)^2)))

RSS_compare <- ref_tac_data_noisecalc %>% 
  select(-ref_tacs, -splinefit) %>% 
  rename(RSS_Ref = RSS) %>% 
  inner_join(target_tac_data_noisecalc %>% select(-target_tacs, -splinefit)) %>% 
  mutate(RSS_Ratio = RSS / RSS_Ref)


ggplot(RSS_compare,
       aes(x=region, y=RSS_Ratio, colour=region)) + 
  ggbeeswarm::geom_beeswarm(cex=2) +
  geom_hline(yintercept=1, linetype="dashed") +
  labs(x = "Region",
       y = "RSS Ratio",
       colour = "Region") +
  scale_y_log10()
```


```{r, eval=use_feng1tc, echo=use_feng1tc, results='asis'}
str_glue("## Feng+1TC Reference Model Fitting

Fitting reference TAC using Feng arterial input function combined with one-tissue compartment model.")
```

```{r feng1tc-fitting, eval=use_feng1tc, echo=use_feng1tc, report_warnings=TRUE}
ref_tac_fitdata <- ref_tac_data %>%
  inner_join(ref_weights_final) %>%
  group_by(across(intersect(colnames(.), c("sub", "ses", "trc", "rec", "task",
                                           "run", "pet", "tac_filename")))) %>%
  nest(.key = "ref_tacs") %>%
  ungroup()

ref_tac_fitdata <- ref_tac_fitdata %>%
  mutate(ref_fit = future_map(ref_tacs, ~feng_1tc_tac(t_tac   = .x$frame_mid/60,
                                              tac     = .x$RefTAC,
                                              weights = .x$ref_weights),
                              .options = furrr_options(seed = TRUE)))

ref_tac_preddata <- ref_tac_fitdata %>%
  mutate(ref_preds = map(ref_fit, ~.x$tacs %>%
                          rename(frame_mid = Time,
                                 RefTAC = Reference,
                                 RefTAC_fitted = Reference_fitted) %>%
                           mutate(frame_mid = frame_mid*60))) %>%
  select(-ref_fit) %>%
  mutate(ref_tacs = map2(ref_tacs, ref_preds, inner_join)) %>%
  mutate(ref_tacs = map(ref_tacs, ~.x %>%
                          select(-any_of("TAC")) %>%
                          rename(RefTAC_original = RefTAC,
                                 RefTAC = RefTAC_fitted,
                                 RefTAC_weights = ref_weights))) %>%
  select(-ref_preds) %>%
  mutate(ref_tacs_json = map(1:n(), ~c(list(
    Description = "Feng+1TC Reference TAC Model fitted values.",
    ModelName = "Feng+1TC",
    FittingMethod = "feng1tc",
    AdditionalModelDetails = str_glue("Reference region: {ref_region_name}. Weighting: {weights_method_display}")),
    bids_column_units(list(
      frame_start = "Frame start time",
      frame_end = "Frame end time",
      frame_dur = "Frame duration",
      frame_mid = "Frame midpoint time",
      RefTAC = "Modelled reference TAC (fitted values for use in modelling)",
      RefTAC_original = "Original measured reference TAC",
      RefTAC_weights = "Weights used for reference TAC fitting"),
      radioactivity_units = tac_units$radioactivity,
      time_units = tac_units$time))))
```


```{r feng1tc-saving, eval=use_feng1tc, echo=use_feng1tc}
# Extract parameters from ref_tac_fitdata (using ref_fit objects)
feng_pars_save <- ref_tac_fitdata %>%
  mutate(kinpars = map(ref_fit, ~.x$par)) %>%
  select(-ref_tacs, -ref_fit) %>%
  unnest(kinpars)

# Get folder info for saving
folder_data <- tac_data %>%
  filter(!duplicated(filename)) %>%
  rename(tac_filename = filename) %>%
  mutate(dirname = dirname(tac_filename)) %>%
  select(pet, dirname)

# Save TSV with parameters for each PET measurement
feng_kinpar_save <- feng_pars_save %>%
  inner_join(folder_data) %>%
  mutate(filename = paste0(analysis_folder, "/", dirname, "/", pet, "_model-feng1tc_desc-ref_kinpar.tsv")) %>%
  nest(kinpar = c(t0, A, B, C, alpha, beta, gamma, Ph1, Th1))

walk2(feng_kinpar_save$kinpar,
      feng_kinpar_save$filename,
      ~write_tsv(.x, .y))

# Save JSON sidecar for parameters (reactive to method settings)
feng_json_output <- list(
  Description = "Feng+1TC Reference TAC Model fitting parameters.",
  ModelName = "Feng+1TC",
  FittingMethod = "feng1tc",
  AdditionalModelDetails = str_glue("Reference region: {ref_region_name}. Weighting: {weights_method_display}"),
  t0 = "Time point at which curve begins to rise (minutes)",
  A = "Feng AIF model amplitude parameter A",
  B = "Feng AIF model amplitude parameter B",
  C = "Feng AIF model amplitude parameter C",
  alpha = "Feng AIF model rate parameter alpha",
  beta = "Feng AIF model rate parameter beta",
  gamma = "Feng AIF model rate parameter gamma",
  Ph1 = "1TC IRF model Phi1 parameter (uptake)",
  Th1 = "1TC IRF model Theta2 parameter (clearance)"
)

feng_json_output <- jsonlite::toJSON(feng_json_output, pretty = TRUE, auto_unbox = TRUE)

# Write JSON for each PET
walk(feng_kinpar_save$filename, ~{
  json_filename <- str_replace(.x, ".tsv", ".json")
  writeLines(feng_json_output, json_filename)
})
```



```{r, eval=use_feng1tc, echo=use_feng1tc, results='asis'}
str_glue("### Feng+1TC Reference Model Plots")
```

```{r feng1tc-plotting, eval=use_feng1tc, echo=use_feng1tc}
walk2(ref_tac_fitdata$ref_fit,
      ref_tac_fitdata$pet, 
      ~print(plot(.x) + labs(title = .y)))
```



```{r, eval=use_spline, echo=use_spline, results='asis'}
# Report what will actually be used: an unset or negative spline_df is a request
# for kinfitr to choose the basis dimension itself, not a request for 5.
spline_df_display <- suppressWarnings(as.numeric(config$ReferenceTAC$spline_df %||% NA_real_))
spline_df_display <- if (is.na(spline_df_display) || spline_df_display < 0) {
  "automatically selected"
} else {
  as.character(spline_df_display)
}

str_glue("## Spline Model Fitting

Fitting reference TAC using smooth spline with **{spline_df_display}** degrees of freedom.")
```

```{r spline-fitting, eval=use_spline, echo=use_spline, report_warnings=TRUE}
spline_df <- as.numeric(config$ReferenceTAC$spline_df %||% -1)

ref_tac_fitdata <- ref_tac_data %>%
  inner_join(ref_weights_final) %>%
  group_by(across(intersect(colnames(.), c("sub", "ses", "trc", "rec", "task",
                                           "run", "pet", "tac_filename")))) %>%
  nest(.key = "ref_tacs") %>%
  ungroup()

ref_tac_fitdata <- ref_tac_fitdata %>%
  mutate(ref_fit = future_map(ref_tacs, ~fit_reference_spline_tac(t_tac     = .x$frame_mid/60,
                                                                  tac       = .x$RefTAC,
                                                                  weights   = .x$ref_weights,
                                                                  spline_df = spline_df),
                              .options = furrr_options(seed = TRUE)))

ref_tac_fitdata <- ref_tac_fitdata %>%
  mutate(fit_message = map_chr(ref_fit, ~.x$fit_message %||% ""))

# Surface any measurement which fell back to the raw reference TAC, rather than
# letting it pass silently as though it had been splined.
spline_fit_messages <- ref_tac_fitdata %>%
  filter(fit_message != "") %>%
  select(any_of(c("sub", "ses", "trc", "rec", "task", "run", "pet")), fit_message)

if (nrow(spline_fit_messages) > 0) {
  print(kable(spline_fit_messages, caption = "Spline fitting fallbacks"))
}

ref_tac_preddata <- ref_tac_fitdata %>%
  mutate(ref_preds = map2(ref_fit, ref_tacs, reference_spline_predictions)) %>%
  select(-ref_fit) %>%
  mutate(ref_tacs = map2(ref_tacs, ref_preds, inner_join)) %>%
  mutate(ref_tacs = map(ref_tacs, ~.x %>%
                          select(-any_of("TAC")) %>%
                          rename(RefTAC_original = RefTAC,
                                 RefTAC = RefTAC_fitted,
                                 RefTAC_weights = ref_weights))) %>%
  select(-ref_preds) %>%
  # The sidecar is written per measurement, so a measurement which fell back
  # says so in its own sidecar instead of claiming a spline it did not get.
  mutate(ref_tacs_json = map(fit_message, ~c(list(
    Description = if (.x == "") {
      "Spline Reference TAC Model fitted values."
    } else {
      "Raw Reference TAC (spline fitting fell back to the unsmoothed data)."
    },
    ModelName = if (.x == "") "Spline" else "Raw",
    FittingMethod = if (.x == "") "spline" else "raw",
    # paste0 rather than glue for the fallback message: it carries the
    # optimiser's own error text, which may contain braces.
    AdditionalModelDetails = paste0(
      str_glue("Reference region: {ref_region_name}. ",
               "Weighting: {weights_method_display}. ",
               "Degrees of freedom: {spline_df}"),
      if (.x == "") "" else paste0(". ", .x))),
    bids_column_units(list(
      frame_start = "Frame start time",
      frame_end = "Frame end time",
      frame_dur = "Frame duration",
      frame_mid = "Frame midpoint time",
      RefTAC = "Modelled reference TAC (fitted values for use in modelling)",
      RefTAC_original = "Original measured reference TAC",
      RefTAC_weights = "Weights used for reference TAC fitting"),
      radioactivity_units = tac_units$radioactivity,
      time_units = tac_units$time))))
```

```{r spline-saving, eval=use_spline, echo=use_spline}
# Extract t0 from spline fits. A measurement which fell back to the raw TAC has
# no fitted t0, and carries its fallback message through so that its sidecar
# describes what it actually got.
spline_pars_save <- ref_tac_fitdata %>%
  mutate(t0 = map_dbl(ref_fit, ~.x$par$t0)) %>%
  select(-ref_tacs, -ref_fit)

# Get folder info for saving
folder_data <- tac_data %>%
  filter(!duplicated(filename)) %>%
  rename(tac_filename = filename) %>%
  mutate(dirname = dirname(tac_filename)) %>%
  select(pet, dirname)

# Save TSV with t0 for each PET measurement
spline_kinpar_save <- spline_pars_save %>%
  inner_join(folder_data) %>%
  mutate(filename = paste0(analysis_folder, "/", dirname, "/", pet, "_model-spline_desc-ref_kinpar.tsv")) %>% 
  nest(kinpar = c(t0))
  

walk2(spline_kinpar_save$kinpar,
      spline_kinpar_save$filename,
      ~write_tsv(.x, .y))

# Save JSON sidecar for parameters, per measurement: one that falls back to the
# raw TAC has no fitted t0 to describe, and should not be labelled a spline.
spline_json_output <- function(fit_message) {
  fell_back <- fit_message != ""

  out <- list(
    Description = if (fell_back) {
      "Raw Reference TAC (spline fitting fell back to the unsmoothed data)."
    } else {
      "Spline Reference TAC Model fitting parameters."
    },
    ModelName = if (fell_back) "Raw" else "Spline",
    FittingMethod = if (fell_back) "raw" else "spline",
    # paste0 rather than glue for the fallback message: it carries the
    # optimiser's own error text, which may contain braces.
    AdditionalModelDetails = paste0(
      str_glue("Reference region: {ref_region_name}. ",
               "Weighting: {weights_method_display}. ",
               "Degrees of freedom: {spline_df}"),
      if (fell_back) paste0(". ", fit_message) else ""),
    t0 = if (fell_back) {
      "Not estimated: no curve was fitted, so t0 is NA."
    } else {
      "Time point at which curve begins to rise (minutes)"
    }
  )

  jsonlite::toJSON(out, pretty = TRUE, auto_unbox = TRUE)
}

# Write JSON for each PET
walk2(spline_kinpar_save$filename, spline_kinpar_save$fit_message, ~{
  json_filename <- str_replace(.x, ".tsv", ".json")
  writeLines(spline_json_output(.y), json_filename)
})
```


```{r, eval=use_spline, echo=use_spline, results='asis'}
str_glue("### Spline Reference Model Plots")
```

```{r spline-plotting, eval=use_spline, echo=use_spline}
pwalk(list(ref_tac_fitdata$ref_fit,
           ref_tac_fitdata$ref_tacs,
           ref_tac_fitdata$pet),
      ~print(plot_reference_spline_fit(..1, ..2) + labs(title = ..3)))
```



# Saving Outcomes

```{r save-reference-tacs}
save_files <- tac_data %>%
  filter(!duplicated(filename)) %>%
  rename(tac_filename = filename) %>%
  select((intersect(colnames(.), c("sub", "ses", "trc", "rec", "task",
                                   "run", "pet", "tac_filename")))) %>%
  inner_join(ref_tac_preddata, by = intersect(colnames(.), colnames(ref_tac_preddata)))

# Save reference TAC TSV files
save_files %>%
  pwalk(function(ref_tacs, tac_filename, ...) {
    reftac_filename <- str_replace(tac_filename,
                                   "desc-combinedregions_tacs.tsv",
                                   "desc-ref_tacs.tsv")
    write_tsv(ref_tacs,
              file = paste0(analysis_folder, "/", reftac_filename))
  })

# Save reference TAC JSON metadata files
save_files %>%
  pwalk(function(ref_tacs_json, tac_filename, ...) {
    reftac_filename <- str_replace(tac_filename,
                                   "desc-combinedregions_tacs.tsv",
                                   "desc-ref_tacs.json")
    write_json(ref_tacs_json,
               path = paste0(analysis_folder, "/", reftac_filename),
               pretty = TRUE,
               auto_unbox = TRUE)
  })
```

## Saving Target TAC Files

The target TAC files contain all regions except the reference region, for use in model fitting.

```{r save-target-tacs}
# Get target TACs (excluding reference region) with filename info
target_tac_files <- tac_data %>%
  filter(region != ref_region_name)

# Save as new targetregions files (preserving original combinedregions files)
target_tac_files %>%
  group_by(filename) %>%
  group_walk(~{
    target_filename <- str_replace(.y$filename,
                                   "desc-combinedregions_tacs.tsv",
                                   "desc-targetregions_tacs.tsv")
    write_tsv(.x,
              file = paste0(analysis_folder, "/", target_filename))
  })
```

```{r save-target-tacs-json}
target_tacs_json <- c(
  list(
    Description = str_glue("Target region TACs for model fitting: the combined ",
                           "regions excluding the reference region."),
    AdditionalModelDetails = str_glue("Reference region excluded: {ref_region_name}")),
  bids_column_units(list(
    region = "Brain region name (combined from constituent regions)",
    frame_start = "Frame start time",
    frame_end = "Frame end time",
    frame_dur = "Frame duration",
    frame_mid = "Frame midpoint time",
    TAC = "Time activity curve value (volume-weighted average over the region)"),
    radioactivity_units = tac_units$radioactivity,
    time_units = tac_units$time))

target_tac_files %>%
  group_by(filename) %>%
  group_walk(~{
    target_filename <- str_replace(.y$filename,
                                   "desc-combinedregions_tacs.tsv",
                                   "desc-targetregions_tacs.json")
    write_json(target_tacs_json,
               path = paste0(analysis_folder, "/", target_filename),
               pretty = TRUE,
               auto_unbox = TRUE)
  })
```

```{r cleanup, include=FALSE}
future::plan(future::sequential)
```

---

# Session Information

```{r session-info}
sessionInfo()
```
