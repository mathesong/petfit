...
# both cases fall back to the weighted mean, which is the honest fit, and say so
# in the report rather than failing the whole step.
  ...
  if (!auto_df && spline_df < 2) {
    return(constant_reference_tac_fit(
      ...
      reason = str_glue("Spline degrees of freedom ({spline_df}) is below 2, ",
                        "which cannot form a spline basis; using a weighted constant fit.")
    ))
  }
  ...
  tryCatch(
    ...
    error = function(e) {
      constant_reference_tac_fit(
        ...
        tac = tac,
        weights = weights,
        reason = str_glue("Spline fit failed ({conditionMessage(e)}); using a weighted constant fit.")
      )
    }
  )
}
...
# Surface any measurement that fell back to a constant fit, rather than letting
# it pass silently as though it had been splined.
