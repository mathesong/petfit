...
weights_tacdata <- tibble(
  ...
  mutate(attributes = map(filename, kinfitr::bids_filename_attributes)) %>%
  unnest(attributes) %>%
  # The key comes from each measurement's own path, so it does not move when the
  # cohort changes. This used to parse the whole study and join the result back
  # on, which made the identifier depend on which other scans were present.
  mutate(pet = petfit::pet_key(filename)) %>%
  group_by(filename) %>% 
...
ggplotly(weights_plot, tooltip = "text", width = 800, height = 500) %>%
...
ggplotly(tac_plot, tooltip = "text", width = 800, height = 500) %>%
  highlight(on = "plotly_hover", off = "plotly_doubleclick", 
           opacityDim = 0.2) %>%
  layout(
    hovermode = "closest",
