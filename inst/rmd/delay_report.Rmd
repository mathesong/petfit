...
if (!is.null(config$FitDelay)) {
  ...
  method_display <- switch(delay_config$model %||% "none",
    ...
    "nested_1tcm" = "Nested 1TCM Shared Delay from Multiple Regions",
    "nested_2tcm" = "Nested 2TCM Shared Delay from Multiple Regions",
    "Unknown method"
  )
...
do_nested_1tcm <- delay_method == "nested_1tcm"
do_nested_2tcm <- delay_method == "nested_2tcm"
```
...
# is_nested: the nested approaches fit all regions of a measurement jointly
# with a single shared delay. They load the same regional TAC data as the
# median approaches.
is_nested <- delay_method %in% c("nested_1tcm", "nested_2tcm")

# Regional TAC loading is shared between the median and nested approaches
load_regional_tacs <- is_multiple_regions || is_nested
```
...
```{r load-tacs-multiple, echo=load_regional_tacs, eval=load_regional_tacs}
...
# Apply the optional region subset for the multiple-regions and nested
# approaches. Uses the same parsing, validation and negation semantics as the
# Subsetting configuration.
delay_region_values <- parse_semicolon_values(
  config$FitDelay$multiple_regions %||% "", field = "Regions")

if (!is.null(delay_region_values)) {
  tac_data <- subset_combined_tacs(tac_data,
                                   list(regions = delay_region_values))
}
```
...
```{r}
# The nested approaches fit all regions of a measurement together, so `region`
# stays inside the nested TAC data rather than in the grouping keys.
nest_keys <- c("sub", "ses", "trc", "rec", "task", "run", "pet", "filename")
if (!is_nested) {
  nest_keys <- c(nest_keys, "region")
}

tac_data_nested <- tac_data %>%
  group_by(across(intersect(colnames(.), nest_keys))) %>%
  nest(.key="tacs")
...
```{r, eval=is_nested, echo=is_nested, results='asis'}
str_glue("## Checking Regions per Measurement

         A nested delay model shares the delay across the regions of each
         measurement, so a measurement with fewer than two regions cannot be
         fitted, and is excluded with a warning.")
```

```{r nested-region-check, eval=is_nested, echo=is_nested, report_warnings=TRUE}
...
<!--                                  vB =  if(fit_vB) NULL else vB_value, -->
...
```{r delay-fit-1tcm-singletac, eval=do_1tcm_singletac, echo=do_1tcm_singletac, report_warnings=TRUE}
...
```{r delay-fit-2tcm-singletac, eval=do_2tcm_singletac, echo=do_2tcm_singletac, report_warnings=TRUE}
...
```{r delay-fit-1tcm-median, eval=do_1tcm_median, echo=do_1tcm_median, report_warnings=TRUE}
...
```{r delay-fit-2tcm-median, eval=do_2tcm_median, echo=do_2tcm_median, report_warnings=TRUE}
...
```{r, eval=do_nested_1tcm, echo=do_nested_1tcm, results='asis'}
str_glue("## Nested 1TCM Shared Delay from Multiple Regions")
```

```{r delay-fit-nested-1tcm, eval=do_nested_1tcm, echo=do_nested_1tcm, report_warnings=TRUE}
# Nested 1TCM delay estimation: all regions of a measurement are fitted
# jointly, sharing a single delay parameter
time_window <- config$FitDelay$time_window %||% 5
vB_value <- config$FitDelay$vB_value %||% 0.05
use_weights <- config$FitDelay$use_weights %||% FALSE
inpshift_lower <- config$FitDelay$inpshift_lower %||% -0.5
...
safe_nested_1tcm_delay <- possibly(nested_1tcm_delay, otherwise = NA)

# multstart_iter = 1 here, unlike the median methods: the nested fit refits
# every region at every outer objective evaluation, so multistart multiplies
# an already large cost, and the outer optimisation already explores the delay.
model_data <- model_data %>%
  mutate(delayFit = future_map2(tacs, input,
                         ~safe_nested_1tcm_delay(t_tac = .x$frame_mid, tac = .x$TAC,
                                 region = .x$region,
                                 input = .y,
                                 weights = if(use_weights) .x$weights else NULL,
                                 vB = vB_value,
...
```{r, eval=do_nested_2tcm, echo=do_nested_2tcm, results='asis'}
str_glue("## Nested 2TCM Shared Delay from Multiple Regions")
```

```{r delay-fit-nested-2tcm, eval=do_nested_2tcm, echo=do_nested_2tcm, report_warnings=TRUE}
# Nested 2TCM delay estimation: all regions of a measurement are fitted
# jointly, sharing a single delay parameter
time_window <- config$FitDelay$time_window %||% 5
vB_value <- config$FitDelay$vB_value %||% 0.05
use_weights <- config$FitDelay$use_weights %||% FALSE
...
safe_nested_2tcm_delay <- possibly(nested_2tcm_delay, otherwise = NA)
...
model_data <- model_data %>%
  mutate(delayFit = future_map2(tacs, input,
                         ~safe_nested_2tcm_delay(t_tac = .x$frame_mid, tac = .x$TAC,
                                 region = .x$region,
                                 input = .y,
                                 weights = if(use_weights) .x$weights else NULL,
                                 vB = vB_value,
...
```{r, eval=is_nested, echo=is_nested}
# The nested fit estimates one shared delay per measurement directly, so no
# median step is needed: the fitted inpshift is constant across regions.
delay_out <- model_data %>%
  ungroup() %>%
  mutate(inpshift = map_dbl(delayFit, ~{
    if(!is.list(.x)) NA_real_ else .x$par$inpshift[1]
  })) %>%
  select(-delayFit, -tacs, -input)
```
...
```{r delay-plot-nested, eval=is_nested, echo=is_nested, fig.width=7, fig.height=4}
# The nested fit already used a single shared delay across regions, so its own
# fit can be plotted directly: all regions with their fits, and the shifted AIF
# as a red dashed line.
walk2(model_data$delayFit, model_data$pet, ~{
  if (is.list(.x)) {
    # Points remain sized by weight, but the size legend adds clutter next to
    # the region legend, so only the regions are shown
    print(plot(.x) +
            labs(title = .y, x = "Time (min)") +
            guides(size = "none"))
  }
})
```

```{r delay-refit-1tcm-median, eval=do_1tcm_median, echo=do_1tcm_median, report_warnings=TRUE}
...
```{r delay-refit-2tcm-median, eval=do_2tcm_median, echo=do_2tcm_median, report_warnings=TRUE}
# fit again with new delay
fit_again_with_newdelay <- model_data %>%
  select(-delayFit) %>%
  inner_join(delay_out) %>%
...
```{r}
# The regex scrape of method_display would yield "Nested" for the nested
# methods, so their model names are set explicitly.
ModelName = switch(delay_method,
                   "nested_1tcm" = "nested1TCM",
                   "nested_2tcm" = "nested2TCM",
                   str_match(method_display, "^(\\w*) ")[,2])
...
save_inpshift <- function(inpshift, filename) {
  ...
  json_output <- c(
    ...
    # The models are fitted in minutes, so the estimated shift is in minutes,
    # even though the TAC and blood files record their times in seconds.
    bids_column_units(list(
      blood_timeshift = paste("Estimated timing offset of the blood data",
                              "relative to the tissue TAC. A positive value",
                              "means that the blood data are shifted later in",
                              "time to align them with the TAC.")),
      time_units = "min")
  )
