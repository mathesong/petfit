...
call_kinfitr_suv <- function(tac_values, tac, frameStartEnd = NULL, timeStartEnd = NULL) {
  suv_args <- list(
    tac = tac_values,
    t_tac = tac$frame_mid,
    dur_tac = tac$frame_dur,
    injRad = 1,
    bodymass = 1
  )

  if (!is.null(frameStartEnd)) {
    suv_args$frameStartEnd <- frameStartEnd
  }

  if (!is.null(timeStartEnd)) {
    if (!"timeStartEnd" %in% names(formals(kinfitr::SUV))) {
      stop("The installed kinfitr::SUV() does not support timeStartEnd. Please update kinfitr to use time-windowed SUVR.")
    }

    suv_args$timeStartEnd <- timeStartEnd
  }

  do.call(kinfitr::SUV, suv_args)
}

scalar_or_default <- function(x, index, default) {
  if (is.null(x) || length(x) < index || is.na(x[[index]])) {
    default
  } else {
    x[[index]]
  }
...
annotate_suvr_window <- function(tac, frameStartEnd = NULL, timeStartEnd = NULL) {
  if (!is.null(frameStartEnd)) {
    frameStartEnd <- c(
      as.integer(scalar_or_default(frameStartEnd, 1, 1)),
      as.integer(scalar_or_default(frameStartEnd, 2, nrow(tac)))
    )
  }

  if (!is.null(timeStartEnd)) {
    timeStartEnd <- c(
      as.numeric(scalar_or_default(timeStartEnd, 1, min(tac$frame_start, na.rm = TRUE))),
      as.numeric(scalar_or_default(timeStartEnd, 2, max(tac$frame_end, na.rm = TRUE)))
    )
  }
  ...
  if (!is.null(frameStartEnd)) {
    tac %>%
      mutate(
        included = frame_index >= frameStartEnd[[1]] & frame_index <= frameStartEnd[[2]],
        window_duration = if_else(included, frame_dur, 0)
      )
  } else if (!is.null(timeStartEnd)) {
    tac %>%
      mutate(
        window_duration = pmax(0, pmin(frame_end, timeStartEnd[[2]]) - pmax(frame_start, timeStartEnd[[1]])),
        included = window_duration > 0
      )
  } else {
    tac %>%
      mutate(
        window_duration = frame_dur,
        included = TRUE
      )
  }
}

estimate_suvr <- function(tac, frameStartEnd = NULL, timeStartEnd = NULL) {
  tac <- tac %>%
    arrange(frame_start) %>%
    mutate(frame_index = row_number())

  if (!is.null(frameStartEnd) && all(as.numeric(frameStartEnd) == 0)) {
    frameStartEnd <- NULL
  }

  if (!is.null(timeStartEnd) && all(as.numeric(timeStartEnd) == 0)) {
    timeStartEnd <- NULL
  }

  target_suv <- call_kinfitr_suv(tac$TAC, tac, frameStartEnd, timeStartEnd)
  reference_suv <- call_kinfitr_suv(tac$RefTAC, tac, frameStartEnd, timeStartEnd)

  windowed <- annotate_suvr_window(tac, frameStartEnd, timeStartEnd)

  selected <- windowed %>%
    filter(included)

  if (!is.null(timeStartEnd)) {
    window_start <- as.numeric(timeStartEnd[[1]])
    window_end <- as.numeric(timeStartEnd[[2]])
  } else if (nrow(selected) > 0) {
    window_start <- min(selected$frame_start, na.rm = TRUE)
    window_end <- max(selected$frame_end, na.rm = TRUE)
  } else {
  ...
  target_auc <- target_suv$par$intSUV
  reference_auc <- reference_suv$par$intSUV
...
model_data <- model_data %>%
  mutate(suvr_result = future_map(
    tacs,
    ~estimate_suvr(.x, frameStartEnd, timeStartEnd),
