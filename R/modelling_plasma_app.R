...
modelling_plasma_app <- function(bids_dir = NULL, derivatives_dir = NULL, blood_dir = NULL, analysis_foldername = "Primary_Analysis", config_file = NULL, cores = 1L, save_logs = FALSE, ancillary_analysis_folder = NULL) {
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$fit_model, {
      ...
      res <- tryCatch(
        withProgress(message = "Fitting model...", value = 0.5, {
          suppressWarnings(fit_single_measurement_plasma(
    ...
    observeEvent(input$tstar_scan_regions, {
      tryCatch({
        tac_files <- list.files(output_dir, pattern = "_desc-combinedregions_tacs.tsv",
                                recursive = TRUE, full.names = TRUE)
        if (length(tac_files) == 0) {
          showNotification("No TAC files found. Run Data Definition first.",
                          type = "warning", duration = 6)
          return()
        }
    ...
    observeEvent(input$run_tstar, {
      ...
      # No delay fit on disk is ambiguous (delay may be set to zero, or the step may
      # not have been run). Warn and proceed with delay = 0, mirroring the reports.
